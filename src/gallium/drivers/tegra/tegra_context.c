				...
				struct tegra_channel **channelp)
{
	...
	err = drm_tegra_channel_open(screen->drm, client, &channel->channel);
	if (err < 0) {
		free(channel);
		return err;
	}

	err = drm_tegra_bo_create(screen->drm, 0, 32 * 4096, &channel->cmdbuf);
	if (err < 0) {
		free(channel);
		return err;
	}

	err = host1x_job_create(channel->channel, &channel->job);
	if (err < 0) {
		free(channel);
		return err;
	}
...
static void tegra_channel_delete(struct tegra_channel *channel)
{
	...
	drm_tegra_channel_close(channel->channel);
	drm_tegra_bo_put(channel->cmdbuf);
	host1x_job_free(channel->job);
	free(channel);
