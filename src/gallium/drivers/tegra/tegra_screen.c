					 ...
					 enum pipe_shader_cap param)
{
	...
	case PIPE_SHADER_VERTEX:
		...
		case PIPE_SHADER_CAP_MAX_ADDRS:
			return 0; /* probably 1, but let's not even go there yet*/
...
static boolean
tegra_screen_fence_signalled(struct pipe_screen *screen,
				struct pipe_fence_handle *fence)
{
	boolean ret;
	fprintf(stdout, "> %s\n", __func__);
	...
	ret = drm_tegra_signaled(tegra_screen(screen)->drm, tegra_fence(fence)->fence);
		...
		uint64_t timeout)
{
	...
	ret = drm_tegra_wait(tegra_screen(screen)->drm, tegra_fence(fence)->fence, timeout);
...
struct pipe_screen *tegra_screen_create(struct drm_tegra *drm)
{
	...
	screen->base.fence_signalled = tegra_screen_fence_signalled;
